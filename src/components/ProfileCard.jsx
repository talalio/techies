...
const ProfileCardExt = ({ picture, name, email, country, city, bio, links}) => {
		...
		<div className="m-3 w-fit h-full min-h-272 flex items-center justify-center bg-primary">
		    <div className="relative w-96 h-auto bg-primary rounded-md pt-24 pb-8 px-4 shadow-md hover:shadow-lg transition flex flex-col items-center">
		        ...
		        <ul className="flex flex-row text-primary-content gap-2 mt-4">
		            <a href={links.twitter} target="_blank"><FontAwesomeIcon icon={faTwitter} /></a>
					<a href={links.github} target="_blank"><FontAwesomeIcon icon={faGithub} /></a>
					<a href={links.discord} target="_blank"><FontAwesomeIcon icon={faDiscord} /></a>
					<a href={links.telegram} target="_blank"><FontAwesomeIcon icon={faTelegram} /></a>
		        </ul>
		    </div>
		</div>
	)
